...
packages <- c("tidyverse", "data.table", "lubridate", "skimr", "psych", "factoextra", "cluster",
              ...
              "DataExplorer", "RANN", "doParallel", "here", "xgboost")
...
## create folders
dir.create(here::here("mlp output"))
dir.create(here::here("mlp output", "1.ClusteringAnalysis"))
dir.create(here::here("mlp output", "2.ExploratoryDataAnalysis"))
dir.create(here::here("mlp output", "3.ModelPerf1_CreateEnsembles"))
dir.create(here::here("mlp output", "4.ModelPerf2_EvaluateOnTest"))


### Read and clean data ====

## read in data from source
RawData <- fread("data/real_estate.csv", 
...
pdf(here::here("mlp output", "1.ClusteringAnalysis", "1.Elbow_Silhoutte_Cluster_Plots.pdf"))
...
# plot missing data
pdf(here::here("mlp output", "2.ExploratoryDataAnalysis", "1.Plot_Missing.pdf"))
...
pdf(here::here("mlp output", "2.ExploratoryDataAnalysis", "2.Feature_Correlations.pdf"))
Final_Data_DumVar %>%
  select(1:5) %>%
  pairs.panels()
...
model_lm2 %>% 
  ...
  write.csv(here::here("mlp output", "2.ExploratoryDataAnalysis", "3.MultipleLinearRegression_Coef.csv"))

# export vif values
write.csv(vif_model_lm2, here::here("mlp output", "2.ExploratoryDataAnalysis", "4.MultipleLinearRegression_VIF.csv"))
...
RMSE(pred_model_lm2, Final_Data_DumVar[, Y.Hse_Price]) %>%
  write.csv(here::here("mlp output", "2.ExploratoryDataAnalysis", "5.MultipleLinearRegression_RMSE.csv"))

# correlation between predictions and real values
cor(pred_model_lm2, unlist(Final_Data_DumVar[, Y.Hse_Price])) %>%
  write.csv(here::here("mlp output", "2.ExploratoryDataAnalysis", "6.MultipleLinearRegression_Corr.csv"))
...
model_results %>% 
  write.csv(here::here("mlp output", "3.ModelPerf1_CreateEnsembles", "1.Models_RMSE.csv"))


## resample models and plot results
resamples <- resamples(model_list)
pdf(here::here("mlp output", "3.ModelPerf1_CreateEnsembles", "2.Models_RMSE_Resamples.pdf"))
...
modelCor(resamples) %>%
  write.csv(here::here("mlp output", "3.ModelPerf1_CreateEnsembles", "3.Models_Corr.csv"))
...
ensemble_1 <- caretEnsemble(model_list, 
                            ...
                            trControl = my_control)
summary(ensemble_1)
pdf(here::here("mlp output", "3.ModelPerf1_CreateEnsembles", "4.Models_Ensemble1_RMSE.pdf"))
plot(ensemble_1)
dev.off()
...
ensemble_2 <- caretStack(model_list, 
                         ...
                         trControl = my_control)
print(ensemble_2) %>%
  capture.output(file = here::here("mlp output", "3.ModelPerf1_CreateEnsembles", "5.Models_Ensemble2_RMSE.csv"))
...
pred_RMSE %>% 
  capture.output(file = here::here("mlp output", "4.ModelPerf2_EvaluateOnTest", "1.ModelPredictions_RMSE.csv"))
...
pred_cor %>%
  capture.output(file = here::here("mlp output", "4.ModelPerf2_EvaluateOnTest", "2.Model_Corr_Pred_vs_Y.csv"))
